...
func calcTree(head *treeNode, fsm *FSM) error {
	stack := [](*treeNode){head}
	for len(stack) > 0 {
		curNode := stack[len(stack)-1]
		if curNode.left.isLeaf() && curNode.right.isLeaf() {
			nameStartEnd, err := applyOperation(fsm.graph, curNode.symbol, curNode.left.symbol, curNode.right.symbol)
			if err != nil {
				return err
			}
			*curNode = treeNode{
	...
	if index := strings.IndexByte(head.symbol, separator); index >= 0 {
		fsm.start = head.symbol[:index]
		fsm.finish = append(fsm.finish, head.symbol[index+1:])
	}
	return nil
}
